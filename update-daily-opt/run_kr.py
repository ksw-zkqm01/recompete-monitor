...
def cmd_merge_store(a):
    ...
    d2 = dst.execute("DELETE FROM contract WHERE period_end IS NULL "
                     "OR period_end = '' OR period_end < ?", (ct_cut,)).rowcount
    dst.commit()
    dst.execute("VACUUM")
    ...
    print(f"OK  정리(-): 마감 {a.bid_keep}일 지난 공고 {d1:,}건 / "
          f"종료 {a.ct_keep}일 지난·기간미상 계약 {d2:,}건")
    print(f"OK  저장소 누적: 공고 {tb:,}건 / 계약 {tc:,}건 -> data/kr_store.sqlite3")
