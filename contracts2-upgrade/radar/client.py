...
class G2BClient:
    ...
    # --- 저수준 -----------------------------------------------------------
    def _get(self, op, params, base=None):
        url = f"{base or self.base}/{op}"
    ...
    def paged(self, op_key, params, num_rows=999, max_pages=2000, op_name=None, base=None):
        op = op_name or OPS[op_key]
        page = 1
        while page <= max_pages:
            items, total = self._items(
                self._get(op, {**params, "numOfRows": num_rows, "pageNo": page}, base=base))
            if not items:
                return
            for it in items:
                yield it
    ...
    def contracts_svc(self, bgn: date, end: date, divs=("용역", "물품"), **kw):
        """계약정보서비스(ao/CntrctInfoService) — 계약기간(cntrctPrd)이 채워지는 원천.
        업무구분(용역/물품)별 오퍼레이션 × 1주 다위 분할."""
        from .config import CNTRCT_SVC_BASE, CNTRCT_SVC_OPS
        for a, b in _chunks(bgn, end, 7):
            for div in divs:
                for it in self.paged(None, {
                        "inqryDiv": "1",
                        "inqryBgnDt": a.strftime("%Y%m%d0000"),
                        "inqryEndDt": b.strftime("%Y%m%d2359")},
                        op_name=CNTRCT_SVC_OPS[div], base=CNTRCT_SVC_BASE, **kw):
                    it["_bsns_div"] = div
                    yield it
