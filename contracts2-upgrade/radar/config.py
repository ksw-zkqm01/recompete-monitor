...
# ---------------------------------------------------------------------------
# 계약정보서비스 (나라장터 계약정보서비스 1.0 docx + 2026-08-31 라이브 실측 확인)
#
# 왜 따로 쓰나: 개방표준서비스의 cntrctPrd(계약기간)는 실측 채움률 1% 미만.
# 계약정보서비스는 용역 79% / 물품 47% 채움 (2026-08-31, 최근 3일 표본 각 300건).
# → 계약만기(D-90) 레이더의 기간 데이터는 이 서비스에서 가져온다.
# ---------------------------------------------------------------------------
CNTRCT_SVC_BASE = "https://apis.data.go.kr/1230000/ao/CntrctInfoService"
CNTRCT_SVC_OPS = {
    "용역": "getCntrctInfoListServc",
    "물품": "getCntrctInfoListThng",
}
# 요청: inqryDiv=1(계약체결일자), inqryBgnDt/inqryEndDt = YYYYMMDDHHMM (실측 동작 확인)
CONTRACT2_MAP = {
    "contract_no":    "dcsnCntrctNo",     # 확정계약번호
    "unty_no":        "untyCntrctNo",     # 통합계약번호
    "contract_name":  "cntrctNm",
    "bsns_div":       "bsnsDivNm",
    "concl_mthd":     "cntrctCnclsMthdNm",
    "lngtrm_div":     "lngtrmCtnuDivNm",
    "concl_date":     "cntrctCnclsDate",
    "period_raw":     "cntrctPrd",        # 예: "2016.05.02 ~ 2016.06.03" / "2026/12/31" / "계약 후 180일"
    "amount":         "thtmCntrctAmt",    # 금차계약금액
    "total_amount":   "totCntrctAmt",
    "contract_instt": "cntrctInsttNm",
    "instt_ofcl":     "cntrctInsttOfclNm",
    "instt_tel":      "cntrctInsttOfclTelNo",
    "bid_notice_no":  "ntceNo",
    "contract_url":   "cntrctDtlInfoUrl",
    "bid_url":        "cntrctInfoUrl",
    "data_bss_date":  "rgstDt",
    # dminsttList / corpList 는 '^' 구분 문자열 → db.upsert_contracts2 에서 파싱
}
